...
parameter Amount initial_state[35];
parameter Real rate_constants[9];
...
Amount s_3_overhanging_DNA_at_resected_DSB_ends;
Amount BRCA1;
Amount DMC1;
Amount RAD51;
Amount ATM;
...
s_3_overhanging_DNA_at_resected_DSB_ends = initial_state[20];
...
reaction_912429_rate = rate_constants[1] * (CDK2^1) * (Meiotic_Holliday_Junction^1) * (MLH3^1) * (MLH1^1) ;
reaction_912363_rate = rate_constants[2] * (PRDM9^1) * (DNA^1) ;
reaction_912458_rate = rate_constants[3] * (DNA^1) * (Meiotic_single_stranded_DNA_complex^1) ;
reaction_912503_rate = rate_constants[4] * (BRCA2^1) * (CDK4^1) * (H2AFX_Nucleosome^1) * (DMC1^1) * (RPA_heterotrimer^1) * (RAD51^1) * (BRCA1^1) * (ATM^1) * (s_3_overhanging_DNA_at_resected_DSB_ends^1) ;
reaction_9023941_rate = rate_constants[5] * (SPO11_double_stand_break^1) ;
reaction_912368_rate = rate_constants[6] * (DNA^1) * (SPO11_Dimer^1) ;
reaction_912496_rate = rate_constants[7] * (MSH4^1) * (MSH5^1) * (Meiotic_D_loop_complex^1) * (BLM^1) * (TOP3A^1) ;
reaction_1214188_rate = rate_constants[8] * (Nucleosome_with_H3K4me2^1) * (AdoMet^2) ;
...
der(HOP2_TBPIP_MND1) = 0;
der(H2AFX_Nucleosome) = 1 * reaction_912503_rate ;
der(s_3_overhanging_DNA_at_resected_DSB_ends) = 1 * reaction_912503_rate + 1 * reaction_9023943_rate ;
